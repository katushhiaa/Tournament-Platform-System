...
public interface ITournamentRepository : IRepository<Tournament, Guid>
{
    ...
    Task<IReadOnlyList<TournamentPreviewDto>> GetForUserAsync(Guid userId, int page, int pageSize);
    Task<IReadOnlyList<TournamentPreviewDto>> GetAllPreviewAsync(int page, int pageSize, bool randomize);
}
